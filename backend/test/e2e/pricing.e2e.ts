 ...
 *   - Pricing quote for user without subscription (PAY_PER_PICKUP)
 *   - Authentication requirement for pricing quote
 *   - Household role requirement for pricing quote
 *   - Job creation with pricing info stored
 *   - Job pricing type as SUBSCRIPTION when covered by subscription
 *   - Pickup consumption when job is covered by subscription
 *   - Pay-per-pickup pricing when subscription pickups exhausted
 *   - Pricing fields in job response (quotedPrice, pricingType, isCoveredBySubscription)
 */
...
describe('E2E: Pricing Integration', () => {
  ...
  afterAll(async () => {
    ...
    // Cleanup plan - delete subscriptions first to avoid foreign key constraint
    if (planId) {
      await dataSource.query(`DELETE FROM "user_subscriptions" WHERE "plan_id" = $1`, [planId]);
      await dataSource
        .createQueryBuilder()
        .delete()
  ...
  describe('GET /subscriptions/pricing-quote', () => {
    it('should return pay-per-pickup pricing for user without subscription', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        subscriptionPrice: expect.any(String),
      });
    ...
    it('should return pay-per-pickup when subscription pickups exhausted', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await request(httpServer)
          ...
          .set('Authorization', `Bearer ${householdToken}`)
          .send({
            scheduledDate: new Date(Date.now() + 86400000 * (4 + i)).toISOString().split('T')[0],
